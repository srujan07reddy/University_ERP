...
export const AdminDashboard = () => {
  ...
  // ---- Modal state (mutually exclusive to prevent visual overlap) ----
  const [modalType, setModalType] = useState<ModalType>('none');
  const isModalOpen = (t: ModalType) => modalType === t;
  ...
  const handleCreateSurvey = () => {
    ...
    const newSurvey = {
      ...
      options: surveyForm.options.split(',').map((o: string) => o.trim()),
      results: {},
      createdBy: 'Admin',
  ...
  const handleAutoGenerateTimetable = () => {
    ...
    setTimeout(() => {
      const absentStaffIds = leaveRequests.filter((r: any) => r.status === 'Approved').map((r: any) => r.senderId);
      const availableStaff = users.filter((u: any) => u.role === 'Faculty' && !absentStaffIds.includes(u.id));
  ...
  const renderStaff = () => {
    const handleApproveAndReassign = (req: any) => {
      ...
      const staff = users.find((u: any) => u.id === req.senderId);
      ...
      <View>
        <View className="bg-white/5 rounded-3xl p-8 border border-white/10 shadow-sm mb-8">
          ...
          <View className="space-y-4">
            {leaveRequests
              .filter((r: any) => r.receiverRole === 'Admin' && r.status === 'Pending')
              .map((req: any) => (
        ...
        <View className="bg-white/5 rounded-3xl p-8 border border-white/10 shadow-sm">
          ...
          <View className="space-y-3">
            {users
              ...
              .map((staff: any) => {
                const isSub = substitutions.some((s: any) => s.staffId === staff.id);
                return (
                  <View key={staff.id} className="flex-row items-center justify-between p-4 bg-white/5 rounded-2xl">
                    <View className="flex-row items-center">
                      ...
                      <View>
                        <Text className="text-white font-semibold">{staff.name}</Text>
                      </View>
                    </View>
                    <TouchableOpacity
                      onPress={() => handleMarkAbsent(staff)}
                      disabled={isSub}
                      className={`${isSub ? 'bg-white/5' : 'bg-red-500/10'} px-6 py-2 rounded-xl`}
                    >
  ...
  return (
    <View style={{ flex: 1 }} className="bg-[#0F172A]">
      {/* No absolute background layer (prevents stacking overlap issues) */}

      <View className="flex-1 flex-row" style={{ minHeight: 0 }}>
        {/* Desktop Sidebar */}
        <View style={{ width: 288, minHeight: 0 }} className="w-72 bg-[#0F172A]/80 border-r border-white/10 p-8 hidden lg:flex">
          ...
          <ScrollView style={{ flex: 1 }} contentContainerStyle={{ paddingBottom: 24 }} showsVerticalScrollIndicator={true} showsHorizontalScrollIndicator={true} className="">
        ...
        <View className="flex-1" style={{ minHeight: 0 }}>
          ...
          <View className="bg-[#0F172A]/40 p-6 flex-row justify-between items-center border-b border-white/10 backdrop-blur-xl">
            <View className="flex-row items-center">
              <TouchableOpacity onPress={() => openModal('mobileMenu')} className="lg:hidden p-2 mr-4 bg-white/5 rounded-xl border border-white/10">
          ...
          <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: 32, paddingBottom: 300 }} showsVerticalScrollIndicator={true} showsHorizontalScrollIndicator={true} className="">
            ...
            {activeTab === 'Analytics' && (
              <AdminAnalyticsTab
                ...
                setGroupMessageModalVisible={(v: boolean) => (v ? openModal('groupMessage') : closeModal())}
                performanceSettings={performanceSettings}
                messagingTargetIds={messagingTargetIds}
                setMessagingTargetIds={setMessagingTargetIds}
                setPerfSettingsModalVisible={(v: boolean) => (v ? openModal('perfSettings') : closeModal())}
              />
            )}
            {activeTab === 'Surveys' && <AdminSurveysTab setSurveyModalVisible={(v: boolean) => (v ? openModal('survey') : closeModal())} />}
            {activeTab === 'Calendar' && <CalendarModule />}
            {activeTab === 'Users' && (
              <AdminUsersTab
                setAppointmentModalVisible={(v: boolean) => (v ? openModal('appointment') : closeModal())}
                setAdmissionModalVisible={(v: boolean) => (v ? openModal('admission') : closeModal())}
              />
            )}
            {activeTab === 'Finance' && (
              <AdminFinanceTab
            ...
            {activeTab === 'StudentsMaster' && (
              <AdminStudentMasterTab
                ...
                setDetailModalVisible={(v: boolean) => (v ? openModal('detail') : closeModal())}
      ...
      {/* Modals (mutually exclusive via modalType) */}

      <Modal
        animationType="slide"
        transparent
        visible={isModalOpen('groupMessage')}
        onRequestClose={closeModal}
      >
        <View className="flex-1 justify-center items-center bg-black/60 p-4">
          <View className="bg-[#1E293B] w-full max-w-xl p-10 rounded-[40px] border border-white/10">
            <Text className="text-white text-3xl font-bold mb-8">Targeted Messaging</Text>
            <View className="space-y-4">
              <View className="flex-row gap-4">
                {['high', 'medium', 'low'].map((seg) => (
                  <TouchableOpacity
                    key={seg}
                    onPress={() => setMessageForm({ ...messageForm, targetGroup: seg })}
                    className={`flex-1 p-4 rounded-2xl border ${messageForm.targetGroup === seg ? 'bg-blue-600 border-blue-500' : 'bg-white/5 border-white/10'}`}
                  >
                    <Text className={`text-center font-bold uppercase text-[10px] ${messageForm.targetGroup === seg ? 'text-white' : 'text-slate-400'}`}>{seg}</Text>
                  </TouchableOpacity>
                ))}
              </View>
              <TextInput
                ...
                onChangeText={(t) => setMessageForm({ ...messageForm, title: t })}
              />
              <TextInput
                placeholder="Content"
                multiline
                ...
                onChangeText={(t) => setMessageForm({ ...messageForm, body: t })}
              />

              <TouchableOpacity onPress={handleGroupMessage} className="bg-blue-600 p-6 rounded-2xl items-center mt-6">
                <Text className="text-white font-bold">Send Private Broadcast</Text>
              </TouchableOpacity>
              <TouchableOpacity onPress={closeModal} className="p-4 items-center">
      ...
      <Modal animationType="slide" transparent visible={isModalOpen('survey')} onRequestClose={closeModal}>
        <View className="flex-1 justify-center items-center bg-black/60 p-4">
          <View className="bg-[#1E293B] w-full max-w-xl p-10 rounded-[40px] border border-white/10">
            <Text className="text-white text-3xl font-bold mb-8">Create Survey</Text>
            <View className="space-y-4">
              <TextInput
                placeholder="Title"
                placeholderTextColor="#64748b"
                ...
                onChangeText={(t) => setSurveyForm({ ...surveyForm, title: t })}
              />
              <TextInput
                ...
                onChangeText={(t) => setSurveyForm({ ...surveyForm, description: t })}
              />
              <TextInput
                ...
                onChangeText={(t) => setSurveyForm({ ...surveyForm, options: t })}
              />
              <TouchableOpacity onPress={handleCreateSurvey} className="bg-blue-600 p-6 rounded-2xl items-center mt-6">
                <Text className="text-white font-bold">Publish Survey</Text>
              </TouchableOpacity>
              <TouchableOpacity onPress={closeModal} className="p-4 items-center">
                <Text className="text-slate-400">Cancel</Text>
              </TouchableOpacity>
      ...
      <Modal animationType="fade" transparent visible={isModalOpen('perfSettings')} onRequestClose={closeModal}>
        <View className="flex-1 justify-center items-center bg-black/60 p-4">
          <View className="bg-[#1E293B] w-full max-w-lg p-10 rounded-[40px] border border-white/10">
            <Text className="text-white text-2xl font-bold mb-6">Performance Definitions</Text>
            <View className="space-y-6">
              <View>
                <Text className="text-green-400 font-bold mb-2">High</Text>
                <TextInput
                  multiline
                  className="bg-white/5 p-4 rounded-2xl border border-white/10 text-white h-20"
                  value={performanceSettings.high}
                  onChangeText={(t) => updatePerformanceSettings({ ...performanceSettings, high: t })}
                />
              </View>
              <View>
                <Text className="text-blue-400 font-bold mb-2">Medium</Text>
                <TextInput
                  multiline
                  className="bg-white/5 p-4 rounded-2xl border border-white/10 text-white h-20"
                  value={performanceSettings.medium}
                  onChangeText={(t) => updatePerformanceSettings({ ...performanceSettings, medium: t })}
                />
              </View>
              <View>
                <Text className="text-red-400 font-bold mb-2">Low</Text>
                <TextInput
                  multiline
                  className="bg-white/5 p-4 rounded-2xl border border-white/10 text-white h-20"
                  value={performanceSettings.low}
                  onChangeText={(t) => updatePerformanceSettings({ ...performanceSettings, low: t })}
                />
              </View>
              <TouchableOpacity onPress={closeModal} className="bg-blue-600 p-5 rounded-2xl items-center">
      ...
      <Modal animationType="slide" transparent visible={isModalOpen('detail')} onRequestClose={closeModal}>
        <View className="flex-1 justify-center items-center bg-black/60 p-4">
          <View className="bg-[#1E293B] w-full max-w-2xl p-10 rounded-[40px] border border-white/10">
            <Text className="text-white text-2xl font-bold mb-8">{selectedUser?.name}</Text>
            <ScrollView style={{ maxHeight: 384, width: '100%' }} contentContainerStyle={{ flexGrow: 1 }} showsVerticalScrollIndicator={true} showsHorizontalScrollIndicator={true} className="">
              {isEditing ? (
                <View className="space-y-4">
                  <TextInput className="bg-white/5 p-4 rounded-2xl border border-white/10 text-white" value={formData.name} onChangeText={(t) => setFormData({ ...formData, name: t })} />
                  <TextInput className="bg-white/5 p-4 rounded-2xl border border-white/10 text-white" value={formData.email} onChangeText={(t) => setFormData({ ...formData, email: t })} />
                </View>
              ) : (
                <View>
                  <Text className="text-slate-400 leading-relaxed">{selectedUser?.role} record details.</Text>
                </View>
              )}
            </ScrollView>
            <View className="flex-row gap-4 mt-6">
              <TouchableOpacity onPress={closeModal} className="flex-1 bg-blue-600 p-4 rounded-2xl items-center">
      ...
      <Modal animationType="slide" transparent visible={isModalOpen('appointment')} onRequestClose={closeModal}>
        <View className="flex-1 justify-center items-center bg-black/60 p-4">
      ...
      <Modal animationType="slide" transparent visible={isModalOpen('admission')} onRequestClose={closeModal}>
        <View className="flex-1 justify-center items-center bg-black/60 p-4">
          <View className="bg-[#1E293B] w-full max-w-4xl p-10 rounded-[40px] border border-white/10 shadow-2xl h-[90%]">
            ...
            <ScrollView className="pr-4" style={{ flex: 1, width: '100%' }} contentContainerStyle={{ flexGrow: 1 }} showsVerticalScrollIndicator={true} showsHorizontalScrollIndicator={true}>
                ...
                <View>
                  ...
                  <View className="flex-row flex-wrap -mx-2">
                      ...
                      <View className="flex-row gap-2">
                        {['Male', 'Female'].map((s) => (
                          <TouchableOpacity
                            ...
                            className={`flex-1 p-3 rounded-xl border ${formData.sex === s ? 'bg-blue-600 border-blue-500' : 'bg-white/5 border-white/10'}`}
                          >
                    ...
                    <View className="w-full md:w-1/2 px-2 mb-4">
                      ...
                      <TextInput placeholder="Standard" placeholderTextColor="#64748b" className="bg-white/5 p-4 rounded-xl border border-white/10 text-white" value={formData.applyingCourse} onChangeText={(t) => setFormData({ ...formData, applyingCourse: t })} />
                    </View>
                    <View className="w-full md:w-1/2 px-2 mb-4">
                      <Text className="text-slate-400 text-xs mb-2">Previous School</Text>
                      <TextInput placeholder="School Name" placeholderTextColor="#64748b" className="bg-white/5 p-4 rounded-xl border border-white/10 text-white" value={formData.previousInstitute} onChangeText={(t) => setFormData({ ...formData, previousInstitute: t })} />
                    </View>
                  </View>
                </View>
      ...
      <Modal animationType="fade" transparent visible={isModalOpen('reassign')} onRequestClose={closeModal}>
        <View className="flex-1 justify-center items-center bg-black/60 p-4">
          <View className="bg-[#1E293B] w-full max-w-lg p-10 rounded-[40px] border border-white/10">
            <Text className="text-white text-2xl font-bold mb-8">Reassign</Text>
            <ScrollView className="mb-6" style={{ maxHeight: 320, width: '100%' }} contentContainerStyle={{ flexGrow: 1 }} showsVerticalScrollIndicator={true} showsHorizontalScrollIndicator={true}>
              {users
                .filter((u: any) => u.role === 'Faculty' && u.id !== selectedAbsentStaff?.id)
                .map((sub: any) => (
                  <TouchableOpacity
                    key={sub.id}
                    onPress={() => confirmReassignment(sub)}
                    className="bg-white/5 p-4 rounded-2xl border border-white/10 mb-3 flex-row items-center justify-between"
                  >
            ...
            <TouchableOpacity onPress={closeModal} className="bg-white/5 p-4 rounded-2xl items-center">
      ...
      <Modal animationType="fade" transparent visible={isModalOpen('mobileMenu')} onRequestClose={closeModal}>
              ...
              <View className="space-y-2">
                {[
                  ...
                  <TouchableOpacity
                    ...
                    className={`p-4 rounded-2xl flex-row items-center ${activeTab === tab.id ? 'bg-blue-600' : ''}`}
                  >
                    <tab.icon color={activeTab === tab.id ? 'white' : '#64748b'} size={20} />
                    <Text className={`font-bold ml-4 ${activeTab === tab.id ? 'text-white' : 'text-slate-400'}`}>{tab.label}</Text>
                  </TouchableOpacity>
